...
export class PVEngine {
  ...
  applySettings(s = {}) {
    ...
    if (s.preset) {
      ...
      if (p) {
        ...
        /* ★ 预设联动网点：halftoneSize 未显式设置（含旧默认 6，视为未自定义）时采用预设密度，
           用户拖动「网点大小」滑块后才以显式值为准 */
        if (this.viewContainer && (!s.halftoneSize || s.halftoneSize === 6)) {
  ...
  _updateActiveNode(index) {
    ...
    // 3. 背景巨型镂空描边字与几何装饰 — ★ 使用 graphicColor 控制装饰图形色
    if (this.decorations) {
      this.decorations.renderForNode(node, this.graphicColor || this.themeColor);
