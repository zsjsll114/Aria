...
/**
 * ★ 节拍估计（folia 作者方案：「先测 BPM，FFT 拿节奏信息」）——
 * 谱通量(spectral flux) onset 包络 → 自相关找主导周期 → BPM + 置信度 + 拍相位。
 * 复用 _computeBandEnergy 的 16-band 帧（零额外 FFT 开销），纯确定性。
 *
 * @param {Array} frames [{ bands: Float32Array(16), energy }]（STFT 帧序列）
 * @param {number} hopSize STFT hop（采样点）
 * @param {number} sampleRate 当前采样率
 * @returns {{bpm:number, confidence:number, beatOffsetSec:number}|null}
 */
function _estimateTempo(frames, hopSize, sampleRate) {
    var n = frames.length;
    if (n < 32 || !sampleRate || sampleRate <= 0) return null;
    var frameDur = hopSize / sampleRate; /* 每帧时长(秒) */
    if (frameDur <= 0) return null;

    /* ── 1. onset 包络：跨 band 的正差分和（谱通量）── */
    var onset = new Float32Array(n);
    for (var f = 1; f < n; f++) {
        var cur = frames[f].bands, prev = frames[f - 1].bands;
        var flux = 0;
    ...
    for (var i = 0; i < n; i++) {
        var s = onset[i], cnt = 1;
        if (i > 0) { s += onset[i - 1]; cnt++; }
        if (i < n - 1) { s += onset[i + 1]; cnt++; }
        smoothed[i] = s / cnt;
    }
    var env = new Float32Array(n);
    var LOCAL = 24; /* ±24 帧 ≈ ±0.75s 局部均值窗 */
    for (var i2 = 0; i2 < n; i2++) {
        var sum = 0, c2 = 0;
        for (var j = Math.max(0, i2 - LOCAL); j <= Math.min(n - 1, i2 + LOCAL); j++) { sum += smoothed[j]; c2++; }
        var v = smoothed[i2] - sum / c2;
        env[i2] = v > 0 ? v : 0;
    }

    /* ── 3. 自相关找主导周期（0.30s~1.05s ≈ 57~200 BPM）── */
    var minLag = Math.max(2, Math.round(0.30 / frameDur));
    var maxLag = Math.min(n - 2, Math.round(1.05 / frameDur));
    if (maxLag <= minLag) return null;
    var bestLag = 0, bestVal = -Infinity;
    var corrSum = 0, corrCnt = 0;
    for (var lag = minLag; lag <= maxLag; lag++) {
        var acc = 0, c3 = 0;
        for (var k = 0; k + lag < n; k++) { acc += env[k] * env[k + lag]; c3++; }
        if (c3 < 8) continue;
        var r = acc / c3;
        corrSum += r; corrCnt++;
        if (r > bestVal) { bestVal = r; bestLag = lag; }
    }
    if (!bestLag || corrCnt === 0) return null;

    var bpm = 60 / (bestLag * frameDur);
    /* 折叠到 70~150（倍频歧义：中速区间最符合主观节拍感） */
    while (bpm < 70) bpm *= 2;
    while (bpm > 150) bpm /= 2;

    /* 置信度：峰值相对相关曲线均值的突出程度，钳制 0~1 */
    var meanVal = corrSum / corrCnt;
    var confidence = meanVal > 0 ? (bestVal - meanVal) / meanVal : 0;
    confidence = Math.max(0, Math.min(1, confidence));

    /* ── 4. 拍相位：按周期对 onset 取栅格和，最大处为第一拍偏移 ── */
    var bestPhase = 0, bestPhaseVal = -1;
    for (var p = 0; p < bestLag; p++) {
        var ps = 0;
        for (var k2 = p; k2 < n; k2 += bestLag) ps += env[k2];
        if (ps > bestPhaseVal) { bestPhaseVal = ps; bestPhase = p; }
    }

    return {
        bpm: Math.round(bpm * 10) / 10,
        confidence: Math.round(confidence * 100) / 100,
        beatOffsetSec: Math.round(bestPhase * frameDur * 1000) / 1000
    };
}
...
if (typeof self !== 'undefined' && typeof window === 'undefined') {
    ...
    self.onmessage = function(e) {
        ...
        try {
            ...
            /* BPM/节拍估计（folia 方案：FFT 帧复用，零额外下载/解码） */
            var tempo = _estimateTempo(frames, HOP_SIZE, dsRate);
            if (tempo && tempo.bpm) log('  BPM: ' + tempo.bpm + ' (置信度 ' + tempo.confidence + ', 拍相位 ' + tempo.beatOffsetSec + 's)');
...
const ChorusDetector = {
        ...
        _detectMainThread(samples, sampleRate, duration, onProgress) {
            ...
            var tempo = _estimateTempo(frames, 512, dsRate);
            if (tempo && tempo.bpm) log('  BPM: ' + tempo.bpm + ' (置信度 ' + tempo.confidence + ')');
    ...
    if (typeof window !== 'undefined') {
        ...
        logInfo('chorusDetector', '[ChorusDetector] 模块已加载 v4.3 (Worker+降采样+band聚合+边界细分+BPM节拍估计)');
    }
    ...
    /* ★ 命名导出：仅供 node 单测（tests/js/test_rhythm_tempo.js）直接调用内部纯函数；
       浏览器侧一律走 ChorusDetector.detect()（Worker 路径） */
    export { _estimateTempo as estimateTempo, _computeBandEnergy as computeBandEnergyFrames };
