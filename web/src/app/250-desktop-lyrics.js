...
/* ★ 字体值解析（2026-09-22 修复「桌面歌词字体只有黑体/宋体生效」）：
   设置下拉存的是短键（serif/kai/hei/fangsong/mono/custom_*），而歌词窗口
   （lyrics.html）把 aria_dtk_fontfamily 的值**直接当 CSS font-family** 用——
   短键不是合法字体名，只有恰好合法的值（serif→衬线=宋体观感）才生效。
   这里统一把短键解析成完整字体栈；旧版完整串原样透传（向后兼容）。 */
const DTK_FAMILY_MAP = {
...
function resolveDtkFontFamily(val, customFonts) {
    if (!val || val === 'default' || val === 'inherit') return null;
    if (DTK_FAMILY_MAP[val]) return DTK_FAMILY_MAP[val];
    if (customFonts && customFonts[val] && customFonts[val].family) {
        return `'${customFonts[val].family}', sans-serif`;
    }
    /* 旧版存过完整串（含逗号/引号）或未知值：原样透传，交给浏览器解析 */
    return val;
}
export { resolveDtkFontFamily };
...
(function initDtkSettingsRows() {
    ...
    const persist = () => {
        try {
            ...
            const ff = resolveDtkFontFamily(dls.fontFamily, (typeof window !== 'undefined' && window.customFonts) || null);
            if (ff) {
                localStorage.setItem('aria_dtk_fontfamily', ff);
            } else {
