/* ============================================================
 * 75-play-mode.js — 由 src/app.js 拆分自动生成（split_app.js）
 * 来源区间: 原第 1773-1797 行 | 单元数: 6
 * 参照源 web/src/app.js 已删除（拆分完成，勿按旧行号定位）；仅改分片
 * ============================================================ */
import { playModeBtn, playModeIcon } from './30-dom-refs.js';

/* 播放方式切换图标路径 */
const playModeIcons = {
            sequence: '<path d="M9.545 14.272a.856.856 0 00.863-.855v-.448c0-1.004.706-1.677 1.785-1.677h5.005v1.362c0 .407.258.664.673.664a.745.745 0 00.49-.183l2.581-2.166c.316-.266.316-.69 0-.955l-2.581-2.183a.745.745 0 00-.49-.183c-.415 0-.672.258-.672.665v1.294h-4.881c-2.217 0-3.628 1.254-3.628 3.213v.597c0 .474.382.855.855.855zm4.864 5.952c.407 0 .664-.257.664-.664v-1.303h4.881c2.225 0 3.628-1.254 3.628-3.213v-.597a.854.854 0 10-1.71 0v.448c0 1.004-.714 1.677-1.793 1.677h-5.006v-1.353c0-.407-.257-.664-.664-.664a.767.767 0 00-.498.182l-2.573 2.175c-.324.257-.315.68 0 .946l2.573 2.192a.807.807 0 00.498.174z" fill-rule="nonzero"></path>',
            loop: '<path d="M9.545 14.272a.856.856 0 00.863-.855v-.448c0-1.004.706-1.677 1.785-1.677h5.005v1.362c0 .407.258.664.673.664a.745.745 0 00.49-.183l2.581-2.166c.316-.266.316-.69 0-.955l-2.581-2.183a.745.745 0 00-.49-.183c-.415 0-.672.258-.672.665v1.294h-4.881c-2.217 0-3.628 1.254-3.628 3.213v.597c0 .474.382.855.855.855zm4.864 5.952c.407 0 .664-.257.664-.664v-1.303h4.881c2.225 0 3.628-1.254 3.628-3.213v-.597a.854.854 0 10-1.71 0v.448c0 1.004-.714 1.677-1.793 1.677h-5.006v-1.353c0-.407-.257-.664-.664-.664a.767.767 0 00-.498.182l-2.573 2.175c-.324.257-.315.68 0 .946l2.573 2.192a.807.807 0 00.498.174z" fill-rule="nonzero"></path><text x="24" y="26" text-anchor="middle" font-size="9" fill="currentColor" stroke="none" font-weight="bold">1</text>',
            random: '<path d="M20.767 20.44a.81.81 0 00.49-.183l2.58-2.174c.316-.266.316-.681 0-.955l-2.58-2.183a.81.81 0 00-.49-.183c-.415 0-.673.258-.673.673v1.245h-1.162c-.739 0-1.195-.233-1.718-.847l-1.527-1.801 1.527-1.81c.54-.63.946-.847 1.677-.847h1.203v1.279c0 .407.258.664.673.664a.801.801 0 00.49-.174l2.58-2.175c.316-.266.316-.69 0-.955l-2.58-2.183a.761.761 0 00-.49-.183c-.415 0-.673.258-.673.665v1.386h-1.212c-1.228 0-1.992.34-2.863 1.386l-1.412 1.668-1.469-1.751c-.805-.946-1.569-1.303-2.747-1.303H8.896c-.53 0-.896.348-.896.838s.365.838.896.838h1.437c.697 0 1.162.225 1.685.847l1.519 1.801-1.52 1.81c-.53.623-.954.847-1.643.847H8.896c-.53 0-.896.348-.896.838s.365.838.896.838h1.536c1.179 0 1.901-.356 2.706-1.303l1.478-1.751 1.444 1.718c.822.98 1.627 1.336 2.822 1.336h1.212v1.412c0 .415.258.672.673.672z"></path>'
        };

const playModeTitles = { sequence: '顺序播放', loop: '单曲循环', random: '随机播放' };

function updatePlayModeIcon() {
            /* ★ 性能（2026-09-23）：同值跳过 innerHTML 重写（setPlayMode 被重复调同值时省一次 DOM 子树替换） */
            const icon = playModeIcons[playMode];
            if (playModeIcon && icon && playModeIcon.innerHTML !== icon) playModeIcon.innerHTML = icon;
            if (playModeBtn) playModeBtn.title = playModeTitles[playMode];
            /* 底栏播放方式文本/图标同步（220 挂载的事件驱动钩子，替代原 500ms 轮询） */
            if (typeof globalThis.__updateBottomPlayMode === 'function') globalThis.__updateBottomPlayMode();
        }

function cyclePlayMode() {
            if (playMode === 'sequence') playMode = 'loop';
            else if (playMode === 'loop') playMode = 'random';
            else playMode = 'sequence';
            updatePlayModeIcon();
        }

function setPlayMode(mode) {
            if (['sequence', 'loop', 'random'].includes(mode)) {
                playMode = mode;
                updatePlayModeIcon();
            }
        }

playModeBtn?.addEventListener('click', cyclePlayMode);

export { cyclePlayMode, playModeIcons, playModeTitles, setPlayMode, updatePlayModeIcon };
