...
/* ★ 必须限定在搜索弹窗内：.source-btn 类名被三处共用（搜索音源导航 data-source /
   榜单源切换 data-rsrc / 自建收藏 tab），全局收集会让 openSearch 的 active 同步清掉
   榜单 tab 的高亮（"没有选中"），音源点击监听还会误绑到榜单 tab 上把 currentSource
   污染成 undefined（"选中漂移"，回搜索页无按钮可匹配）。 */
const sourceBtns = typeof document !== 'undefined' ? document.querySelectorAll('#searchOverlay .source-btn') : null;
