...
import { applyPerformanceProfile, autoDetectAndApplyPerformance, getPerformanceSettings, getPerfVfx, getVfxOverrides, initDefaultSong, loadSettings, savePerformanceSettings, saveSettings, setVfxOverride, showPerformanceDialog } from './180-boot-config.js';
...
document.getElementById('hwCpu').textContent = hw.cpuCores + ' 核';
document.getElementById('hwGpu').textContent = hw.gpu || '未知';
...
document.getElementById('hwCpu').textContent = (navigator.hardwareConcurrency || '?') + ' 核';
document.getElementById('hwGpu').textContent = '点击检测以获取';
document.getElementById('hwResolution').textContent = window.screen.width + 'x' + window.screen.height;
            ...
            const effectsHtml = `
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                    <span style="color: rgba(255,255,255,0.5)">动态背景</span>
                    <span>${p.background.dynamicBg ? (p.background.swayEnabled ? '开（带摇摆）' : '开') : '关'}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                    <span style="color: rgba(255,255,255,0.5)">背景模糊</span>
                    <span>${p.background.blur}px</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                    <span style="color: rgba(255,255,255,0.5)">歌词模糊</span>
                    <span>${p.lyrics.blurLevel}级</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                    <span style="color: rgba(255,255,255,0.5)">显示翻译</span>
                    <span>${p.lyrics.showTranslation ? '是' : '否'}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                    <span style="color: rgba(255,255,255,0.5)">显示罗马音</span>
                    <span>${p.lyrics.showRomaji ? '是' : '否'}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                    <span style="color: rgba(255,255,255,0.5)">毛玻璃强度</span>
                    <span>${p.interface.glassStrength}px</span>
                </div>
                <div style="display: flex; justify-content: space-between;">
                    <span style="color: rgba(255,255,255,0.5)">紧凑模式</span>
                    <span>${p.interface.compactMode ? '是' : '否'}</span>
                </div>
            `;
            ...
            function showTunnelAiConfirm(onReady, lineCount, hasKey, opts) {
                ...
                overlay.innerHTML = `
                    <div class="tunnel-ai-confirm-modal">
                        <div class="tunnel-ai-confirm-title">「蒙德里安拼画 PV」需要 AI 分析歌词以生成最佳视觉效果</div>
...
setTimeout(() => { if (typeof flyinAutoScaleFont === 'function') flyinAutoScaleFont(); }, 100);
            ...
            /* 同步原播放方式变化 */
            setInterval(updateBottomPlayMode, 500);
