...
/* ★ 将相机阻尼系数 wcLerpFactor 换算为缓动补间时长（秒）。
           ...
           ★ 2026-09-21 系数 0.0166667 → 0.0105（整体 ×0.63）：词云焦点逐字变化都会
           重启补间，快歌一字 200~300ms 而旧默认 0.04（≈0.41s）补间走不完就被打断，
           镜头永远慢半拍（"切过去都唱完几个词了"）。现默认 0.04 ≈ 0.26s，配合
           easeOutQuint 前段更快；滑块范围 0.01~0.15 仍映射约 1.05s ~ 0.08s。
           时长有界 → 补间必然在有限时间精确收敛停稳，不会无限逼近导致持续重栅格化。 */
function wcLerpToDuration(lerp) {
            let f = parseFloat(lerp);
            if (!isFinite(f) || f <= 0) f = 0.04;
            f = Math.min(0.5, Math.max(0.005, f));
            return Math.min(2.5, Math.max(0.08, 0.0105 / -Math.log(1 - f)));
        }
