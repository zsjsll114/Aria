...
function setBlurBackground(imageUrl) {
            ...
            const blurFilter = playerConfig.enableBlurBackground
                ? `blur(${blurPx}px) brightness(${brightness})`
                : 'none';

            /* 将新图片设置到非活动层 */
            inactiveLayer.style.backgroundImage = `url("${imageUrl}")`;
            inactiveLayer.style.filter = blurFilter;
...
export { applyColorOverlay, applyColorToLyrics, extractDominantColor, getActiveBgLayer, getActiveCover, getInactiveBgLayer, getInactiveCover, initLyricsInteractions, sanitizeImageUrl, setBlurBackground, setCoverImage };
