...
async function detectHardware() {
            ...
            try {
                ...
                if (gl) {
                    ...
                    if (debugInfo) {
                        ...
                        /* 检测低端 GPU */
                        const lowEndGPUs = ['intel', 'hd graphics', 'basic', 'microsoft', 'software', 'swiftshader', 'llvmpipe'];
                        hw.isLowPower = lowEndGPUs.some(g => gpuLower.includes(g));
                        /* 检测高端 GPU */
                        const highEndGPUs = ['nvidia', 'rtx', 'gtx 16', 'gtx 10', 'amd', 'radeon rx'];
                        const isHighEnd = highEndGPUs.some(g => gpuLower.includes(g));
                        hw.gpuTier = isHighEnd ? 'high' : hw.isLowPower ? 'low' : 'medium';
                        /* ★ 软件渲染识别：WebGL 正常但走软件光栅化（VMware SVGA / SwiftShader / llvmpipe） */
                        const softwareRenderers = ['software', 'swiftshader', 'llvmpipe', 'microsoft basic', 'basic render'];
            ...
            if (hw.isSoftwareRenderer) score = Math.min(score, 4);
