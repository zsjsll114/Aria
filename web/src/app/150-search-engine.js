...
if (typeof window !== "undefined") window.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                if (addToPlaylistOverlay.classList.contains('visible')) closeAddToPlaylist();
                if (importPlaylistOverlay.classList.contains('visible') && !isImportingPlaylist) importPlaylistOverlay.classList.remove('visible');
                if (playlistsOverlay.classList.contains('visible')) closePlaylists();
            }
        });
...
async function fetchSourcePage(source, word, page) {
    ...
    let json = { code: 0 };
    try {
        const res = await proxyFetch(`${API_BASE}/${source}?word=${encodeURIComponent(word)}&num=${SOURCE_PAGE_SIZE[source]}&page=${page}`);
