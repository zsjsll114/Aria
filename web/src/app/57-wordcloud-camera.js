...
class WaveLyricSystem {
    ...
    step(dt) {
        ...
        /* ★ 行高节流重测：每 30 帧（≈0.5s@60fps）重读可见行真实高度，
           覆盖换行/字号/翻译开关等导致的 nd.h 过期（旧高撑不出新行距 → 重叠） */
        if (++this._remeasureTick >= 30) { this._remeasureTick = 0; this.remeasureVisible(); }
...
function updateWordcloudCamera(t) {
            ...
            if (wcTween && wcTween.sig !== sig) {
                ...
                wcTween = {
                    sig,
                    t0: performance.now(),
                    dur: tweenDur,                   /* 由阻尼系数决定：值越小镜头越"重"越慢 */
            ...
            if (wcTween) {
                ...
                const k = tt >= 1 ? 1 : (1 - Math.pow(1 - tt, 4));   /* easeOutQuart：0→1，先快后慢更明显 */
