...
/* ★ 歌单弹窗头部上下文按钮（AI 分析 / 保存队列为歌单）：在具体歌曲列表视图（自建歌单/vendor平台歌单）显示。
   歌单主页面显示的是一个个歌单卡片，非详情视图一律隐藏。 */
function syncPlaylistHeaderCtxBtns() {
  const isSongsList = (playlistViewMode === 'detail' || playlistViewMode === 'selfplat-songs');
  if (playlistSaveQueueBtn) playlistSaveQueueBtn.style.display = (playlistViewMode === 'detail') ? '' : 'none';
  const aiBtn = typeof document !== 'undefined' ? document.getElementById('playlistAiAnalyzeBtn') : null;
  if (aiBtn) aiBtn.style.display = (isSongsList && currentPlaylistId !== '__local_music__') ? '' : 'none';
}
Aria.__syncPlaylistHeaderCtxBtns = syncPlaylistHeaderCtxBtns;

/* ★ 自建平台视图竞态闸：任何切入非自建平台视图的函数先递增 selfPlatSeq，
   让飞行中的 renderSelfPlatList/renderSelfPlatSongs await 落地后因 token 不符而放弃渲染。
   此前只在自建平台视图之间互斥，返回主页面不递增 → QQ 歌单列表请求落地时覆盖了
   已渲染的歌单主页面（标题"歌单"+内容却是平台歌单，且返回键已隐藏无法再返回）。 */
function invalidateSelfPlatRenders() { selfPlatSeq++; }
...
async function renderPlaylistsView() {
            ...
            playlists.forEach(pl => {
                ...
                const coverHtml = pl.cover
                    ? `<img src="${escapeHtml(pl.cover)}" alt="" loading="lazy">`
...
function buildSelfPlatCards(meta) {
  ...
  /* ★ 布局切换按钮只在 ≥2 张卡时显示：单张卡在"网格(1fr槽位≈130px)"与
...
function _renderSelfPlatSongsList(songs, preserveScroll) {
  ...
  playlistsListEl.innerHTML = `
    <div class="rank-list-head" style="display:flex;gap:8px;align-items:center;margin-bottom:6px;">
      ...
      <button class="setting-btn icon-btn" id="selfPlatDl" style="width:34px;height:34px;padding:0;display:inline-flex;align-items:center;justify-content:center;" title="批量下载整单歌曲" aria-label="批量下载">
...
function renderRecentDetail() {
  ...
  const rows = list.map((it, i) => {
    ...
    return `<div class="result-item${np}" data-rh="${i}">
      ${cover ? `<img class="result-cover" src="${escapeHtml(cover)}" alt="" loading="lazy" onerror="this.style.visibility='hidden'">` : `<div class="result-cover" style="display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.1);"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="2"><path d="M9 18V5l12-2v13"></path><circle cx="6" cy="18" r="3"></circle><circle cx="18" cy="16" r="3"></circle></svg></div>`}
      <div class="result-info">
      ...
      <div class="result-actions">
        <button class="fav-item-delete" data-rhdel="${i}" title="删除这条记录">
  ...
  playlistsListEl.innerHTML = `
    <div class="rank-list-head" style="display:flex;gap:8px;align-items:center;margin-bottom:6px;">
      ...
      <button class="setting-btn" id="rcClearBtn" title="清空全部播放记录" style="padding:6px 10px;white-space:nowrap;">清空记录</button>
    </div>${rows}`;
  /* ★ 历史记录删除：单条 × / 清空（此前历史只能看不能删） */
  const _syncRecentMeta = () => {
    const metaEl = document.getElementById('recentEntryMeta');
    const n = (typeof window.getRecentHistory === 'function') ? (window.getRecentHistory() || []).length : 0;
    if (metaEl) metaEl.textContent = n > 0 ? `${n} 首 · 点击查看` : '暂无播放记录';
  };
  playlistsListEl.querySelectorAll('[data-rhdel]').forEach(btn => {
    btn.onclick = (e) => {
      e.stopPropagation();
      const i = Number(btn.dataset.rhdel);
      const cur = (typeof window.getRecentHistory === 'function') ? (window.getRecentHistory() || []) : [];
      if (i < 0 || i >= cur.length) return;
      cur.splice(i, 1);
      if (typeof window.saveRecentHistory === 'function') window.saveRecentHistory(cur);
      renderRecentDetail();
      _syncRecentMeta();
    };
  });
  const _rcClear = document.getElementById('rcClearBtn');
  if (_rcClear) {
    _rcClear.onclick = () => {
      const doClear = () => {
        if (typeof window.saveRecentHistory === 'function') window.saveRecentHistory([]);
        renderRecentDetail();
        _syncRecentMeta();
      };
      if (typeof window.showGlassConfirm === 'function') {
        window.showGlassConfirm({ title: '清空播放记录', desc: '确定要清空全部历史播放记录吗？此操作不可撤销。', danger: true })
          .then(ok => { if (ok) doClear(); });
      } else if (window.confirm('清空全部历史播放记录？')) doClear();
    };
  }
...
function renderNowPlayingDetail() {
            ...
            html += `
                <div class="rank-list-head" style="display:flex;gap:8px;align-items:center;margin-bottom:6px;">
                    ...
                    ${playlist.length >= 2 ? `<button class="setting-btn icon-btn" id="npShuffleBtn" title="随机排序（打乱当前队列）" aria-label="随机排序" style="width:34px;height:34px;padding:0;display:inline-flex;align-items:center;justify-content:center;">
            ...
            Aria.__applySongViewTo(playlistsListEl);
            /* ★ 随机排序（当前播放页）：复用 245 队列面板的洗牌逻辑，只洗队列不动歌单 */
            const _npShuffle = document.getElementById('npShuffleBtn');
            if (_npShuffle) {
                _npShuffle.onclick = () => {
                    if (typeof globalThis.__shufflePlayQueue === 'function') globalThis.__shufflePlayQueue();
...
function renderPlaylistDetail(playlistId) {
                ...
                <div class="rank-list-head" style="display:flex;gap:8px;align-items:center;margin-bottom:6px;">
                    ...
                    ${bEdit
                        ...
                        : `<button class="setting-btn" id="plBatchToggle" style="padding:6px 10px;white-space:nowrap;">多选</button>
                           <button class="setting-btn icon-btn" id="plShuffleBtn" title="随机排序（打乱本歌单顺序）" aria-label="随机排序" style="width:34px;height:34px;padding:0;display:inline-flex;align-items:center;justify-content:center;">
                             <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 3 21 3 21 8"></polyline><line x1="4" y1="20" x2="21" y2="3"></line><polyline points="21 16 21 21 16 21"></polyline><line x1="15" y1="15" x2="21" y2="21"></line><line x1="4" y1="4" x2="9" y2="9"></line></svg>
                           </button>
                           <button class="setting-btn icon-btn" id="plBatchDl" title="批量下载整单歌曲" aria-label="批量下载" style="width:34px;height:34px;padding:0;display:inline-flex;align-items:center;justify-content:center;">
                             <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
                           </button>
                           <button class="setting-btn icon-btn" id="plMergeBtn" title="合并到其它歌单" aria-label="合并到其它歌单" style="width:34px;height:34px;padding:0;display:inline-flex;align-items:center;justify-content:center;">
                             <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 11H5m14 0l-4-4m4 4l-4 4"></path><path d="M5 19h9"></path><path d="M5 5h9"></path></svg>
                           </button>`}
                </div>`;
            ...
            /* ★ 随机排序：Fisher-Yates 打乱歌单本身并保存；编辑模式不渲染此按钮，
               勾选索引（plMultiSel / data-sidx）以渲染时的顺序为准，洗牌会错位 */
            const _shuffleBtn = document.getElementById('plShuffleBtn');
            if (_shuffleBtn) {
                _shuffleBtn.onclick = () => {
                    const plRef = getPlaylists().find(p => p.id === playlistId);
                    if (!plRef || plRef.songs.length < 2) return;
                    const arr = plRef.songs;
                    for (let i = arr.length - 1; i > 0; i--) {
                        const j = Math.floor(Math.random() * (i + 1));
                        const tmp = arr[i]; arr[i] = arr[j]; arr[j] = tmp;
                    }
                    if (arr.length > 0) plRef.cover = arr[0].cover || '';
                    savePlaylists(getPlaylists());
                    globalThis.plMultiSel = new Set();
                    renderPlaylistDetail(playlistId);
                    if (typeof window.showToast === 'function') window.showToast('歌单已随机排序');
                };
            }
            if (_batchDl) {
                _batchDl.onclick = () => {
