...
export const SETTINGS_NAV = [
  { group: '外观', items: [
    { tab: 'appearance', label: '视觉模式' },
    { tab: 'background', label: '背景' },
    { tab: 'fonts',      label: '字体' },
  ] },
  { group: '播放', items: [
    { tab: 'playback',    label: '播放' },
    { tab: 'quality',     label: '音质' },
    { tab: 'audio',       label: '音效' },
    { tab: 'performance', label: '性能' },
  ] },
  { group: '服务', items: [
    { tab: 'selfhost', label: '自建服务' },
    { tab: 'ai',       label: 'AI 分析' },
    { tab: 'data',     label: '数据' },
  ] },
  { group: '系统', items: [
    { tab: 'interface', label: '界面' },
    { tab: 'shortcuts', label: '快捷键' },
    { tab: 'about',     label: '关于' },
  ] },
];
...
export function renderSettingsNav(container) {
  ...
  container.innerHTML = `
    <input class="settings-nav-search" id="settingsNavSearch" type="text"
           placeholder="搜索设置…" autocomplete="off" />
    <div class="settings-nav-list">
      ${SETTINGS_NAV.map((g) => `
        <div class="settings-nav-group">
          <div class="settings-nav-group-title">${g.group}</div>
          ${g.items.map((it, i) => `
            <button class="settings-tab${g === SETTINGS_NAV[0] && i === 0 ? ' active' : ''}" data-tab="${it.tab}" title="${it.label}">
              <span>${it.label}</span>
            </button>`).join('')}
        </div>`).join('')}
    </div>`;
