...
**本地优先的多源音乐播放器：逐字歌词 · 九种歌词视觉 · 桌面歌词 · AI 情绪分析**
...
<!-- push 到 GitHub 后补：Release / CI 构建状态徽章（需要仓库 URL） -->
...
| AI 情绪分析 | Gemini 逐句情感标注、高潮段落识别、构图与配色随情绪编排 |
...
下载发布页的绿色版压缩包，解压后双击「启动Tauri桌面版.bat」。
...
1. **页面地址用 `http://localhost:8001`**，不要用 `127.0.0.1`。AI 功能按页面来源校验，用 127.0.0.1 打开会得到 `403 Forbidden`，AI 情绪分析全部失效。
...
「设置 → AI」里填 Gemini API Key（走 Cloudflare Worker 反代，见 `docs/cf-gemini-auth-worker.js`）。Key 只存在本机 `user_config.json`，不上传到任何第三方。

## 已知限制

诚实列出当前边界，避免重复提 issue：
...
- **可变字体（VF）做全局字体时，全屏大字的插值渲染开销较高**——低端设备建议使用静态字重版本（如思源黑体 Bold），或把性能档调低。
